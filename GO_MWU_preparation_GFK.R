...
#### WeakleyTide vs. LongTideRunoff ####
WTvsLTR_pairwisefiltered_ALLGENES <- read.csv("C:\\Users\\antre\\Box\\Bernal_lab\\Antrelle\\GulfKillifish\\LongTideRunoff_vs_WeakleyTide_ALLGENES_pairwisejoined_filtered.csv", header = TRUE, sep = ",", fill = TRUE)
colnames(WTvsLTR_pairwisefiltered_ALLGENES)
#   [1] "EnsemblID"      "log2FoldChange" "padj"           "Gene_name"      "GO_ID"
#Subset the data to only keep the columns we want: EnsemblID and log2FoldChange
WTvsLTR_EnsemblLogFC_ALLGENES <- WTvsLTR_pairwisefiltered_ALLGENES[c("EnsemblID", "log2FoldChange")]
#Filter data to remove repeated lines
WTvsLTR_Ensembl_LogFC_ALLGENES <- WTvsLTR_EnsemblLogFC_ALLGENES %>%
  distinct()
# Extract table as a .csv file
write.table(WTvsLTR_Ensembl_LogFC_ALLGENES, "WTvsLTR_Ensembl_LogFC_ALLGENES.csv", sep = ",", quote = F) #renamed to WTvsLTR_ELFC_AG.csv for GO_MWU_GFKanalysis
